...
public class OrcidService extends RestSource {
	...
	private final MediaType APPLICATION_ORCID_XML = new MediaType(
			"application", "orcid+xml");
	private final MediaType APPLICATION_VDN_ORCID_XML = new MediaType(
			"application", "vdn.orcid+xml");
	...
	public static OrcidService getOrcid() {
		if (orcid == null) {
			orcid = new DSpace().getServiceManager().getServiceByName(
					"OrcidSource", OrcidService.class);
	...
	public OrcidProfile getProfile(String id, String token) {
		...
		WebTarget target = (WebTarget) restConnector.get(id + READ_PROFILE_ENDPOINT); 
		String response;
		if (token != null) {
	...
	public OrcidWorks getWorks(String id, String token) {
		...
		WebTarget target = (WebTarget) restConnector.get(id+READ_WORKS_ENDPOINT);
	...
	public FundingList getFundings(String id, String token) {
		...
		//WebTarget target = restConnector.getClientRest(id + READ_FUNDING_ENDPOINT);
		WebTarget target = (WebTarget) restConnector.get(id + READ_FUNDING_ENDPOINT);
		String response = target.request()
				.header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
				.accept(MediaType.APPLICATION_XML).acceptEncoding("UTF-8")
				.get(String.class);
		...
		return message.getOrcidProfile().getOrcidActivities().getFundings();
	}
	 ...
	 */
	public String buildProfile(OrcidProfile profile) throws IOException,
			JAXBException {
		//WebTarget target = restConnector.getClientRest(PROFILE_CREATE_ENDPOINT);
		WebTarget target = (WebTarget) restConnector.get(PROFILE_CREATE_ENDPOINT);
		...
		Builder builder = target
				...
				.header(HttpHeaders.AUTHORIZATION,
						"Bearer "
								+ getMemberProfileCreateToken()
										.getAccess_token());
		Entity<String> entity = Entity.entity(sw.toString(),
				APPLICATION_VDN_ORCID_XML);
		...
		if (response.getStatus() != HttpStatus.SC_OK
			...
			if (response.hasEntity()) {
				try {
					...
					OrcidMessage message = (OrcidMessage) um
							.unmarshal((InputStream) response.getEntity());
					log.error(message.getErrorDesc().getContent());
				} catch (JAXBException e) {
					log.info("Problem unmarshalling return value " + e);
					throw new IOException(e.getMessage(), new RuntimeException(
	 ...
	 * @throws JAXBException
	 */
	public OrcidMessage updateBio(String id, String token, OrcidProfile profile)
			throws IOException, JAXBException {
		//WebTarget target = restConnector.getClientRest(id + BIO_UPDATE_ENDPOINT);
		WebTarget target = (WebTarget) restConnector.get(id+BIO_UPDATE_ENDPOINT);
		...
		Builder builder = target.request().accept(MediaType.APPLICATION_XML)
				.acceptEncoding("UTF-8")
				.header(HttpHeaders.AUTHORIZATION, "Bearer " + token);
		Entity<String> entity = Entity.entity(sw.toString(),
				APPLICATION_VDN_ORCID_XML);
	 ...
	 * @throws JAXBException
	 */
	public OrcidMessage updateAffiliations(String id, String token,
			OrcidProfile profile) throws IOException, JAXBException {
		//WebTarget target = restConnector.getClientRest(id + AFFILIATION_UPDATE_ENDPOINT);
		WebTarget target = (WebTarget) restConnector.get(id+AFFILIATION_UPDATE_ENDPOINT);
	...
	private void validate(Marshaller marshaller) throws FileNotFoundException {
		...
		try {
			InputStream inputStream = OrcidService.class
					.getResourceAsStream("orcid-message-" + MESSAGE_VERSION
							+ ".xsd");
			if (inputStream == null) {
	 ...
	 * @throws IOException
	 */
	public OrcidSearchResults search(String query, int page, int pagesize)
			throws IOException {
		...
		WebTarget target = (WebTarget) restConnector.get(SEARCH_ENDPOINT);
		...
		try {
			...
			builder = builder.header(HttpHeaders.AUTHORIZATION, "Bearer "
					+ getMemberSearchToken().getAccess_token());
			reader = new StringReader(builder.get(String.class));
		}
	...
	private OrcidAccessToken getAccessToken(String code, String codeOrScope,
		...
		return new ObjectMapper().reader(OrcidAccessToken.class).readValue(
				response);
	}
	...
	public List<AuthorityValue> queryAuthorities(String field, String text,
		...
		OrcidSearchResults results = search(
				field + ":" + URLEncoder.encode(text), start, max);
	...
	public List<AuthorityValue> queryAuthorities(String text, int max)
			/*throws IOException*/ {
		OrcidSearchResults results = null;
		try {
			results = search(URLEncoder.encode(text), 0, max);
		} catch (IOException e) {
	 ...
	 * @throws JAXBException
	 */
	public OrcidMessage appendWorks(String id, String token, OrcidWorks works)
			throws IOException, JAXBException {
	 ...
	 * @throws JAXBException
	 */
	public OrcidMessage putWorks(String id, String token, OrcidWorks works)
			throws IOException, JAXBException {
		return pushWorks(id, token, works, ORCID_MODE_UPDATE);
	}

	private OrcidMessage pushWorks(String id, String token, OrcidWorks works,
			String method) throws IOException, JAXBException {
		WebTarget target = restConnector.getClientRest(id
				+ WORK_CREATE_ENDPOINT);
	...
	/**
	 * Adds one "work" research activities to the ORCID Record for the scholar
	 * represented by the specified orcid_id.
	 * 
	 * Member API, require '/orcid-works/create' scope.
	 * 
	 ...
	 * @throws JAXBException
	 */
	public String appendWork(String id, String token, OrcidWork work,
			String handle) throws IOException, JAXBException {
		WebTarget target = restConnector.getClientRest(id
				+ WORK_CREATE_ENDPOINT);
		...
		try {
			putCode = message.getOrcidProfile().getOrcidActivities()
					.getOrcidWorks().getOrcidWork().get(0).getPutCode()
					.toString();
		} catch (Exception e) {
	...
	// Fundings
	/**
	 * Adds more "fundings" research activities to the ORCID Record for the
	 ...
	 * @throws JAXBException
	 */
	public OrcidMessage appendFundings(String id, String token,
			FundingList fundings) throws IOException, JAXBException {
	 ...
	 * @throws JAXBException
	 */
	public OrcidMessage putFundings(String id, String token,
			FundingList fundings) throws IOException, JAXBException {
		return pushFundings(id, token, fundings, ORCID_MODE_UPDATE);
	}

	private OrcidMessage pushFundings(String id, String token,
			FundingList fundings, String method) throws IOException,
			JAXBException {
		WebTarget target = restConnector.getClientRest(id
				+ FUNDING_CREATE_ENDPOINT);
	...
	public String appendFunding(String id, String token, Funding funding,
		...
		if (message == null) {
			...
			if (StringUtils.isNotEmpty(uuid)) {
				...
				if (fundings != null) {
					...
					for (Funding justWork : fundings.getFundings()) {
						if (StringUtils.equals(justWork.getSource()
								.getSourceName().getContent(),
								getSourceClientName())) {
		...
		try {
			putCode = message.getOrcidProfile().getOrcidActivities()
					.getFundings().getFundings().get(0).getPutCode()
					.toString();
		} catch (Exception e) {
			log.warn("No put-code for owner for orcid " + id);
		}
	...
	public OrcidBio getBio(String id, String token) {
		...
		WebTarget target = restConnector.getClientRest(id + READ_BIO_ENDPOINT);
		String response;
		if (token != null) {
			response = target.request()
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
	...
	private OrcidMessage getOrcidMessage(Response response) {
		...
		if (response.hasEntity()) {
			try {
				Unmarshaller um = orcidMessageContext.createUnmarshaller();
				message = (OrcidMessage) um.unmarshal((InputStream) response
						.getEntity());
		...
		if (response.getStatus() != HttpStatus.SC_OK
				&& response.getStatus() != HttpStatus.SC_CREATED) {
			if (message != null
					&& message.getErrorDesc() != null
					&& StringUtils.isNotBlank(message.getErrorDesc()
							.getContent())) {
	...
	public static void main(String[] args) throws JAXBException, IOException,
		...
		if (line.hasOption('i') && line.hasOption('s')) {
			...
			System.out
					.println("Try to validate against ORCID MEMBER API 'http://api.orcid.org/v1.2' and ORCID MEMBER TOKEN URL 'https://api.orcid.org/oauth/token'");
			...
			OrcidService orcidService = new OrcidService(
					"http://api.orcid.org/v1.2", clientid, secretid,
