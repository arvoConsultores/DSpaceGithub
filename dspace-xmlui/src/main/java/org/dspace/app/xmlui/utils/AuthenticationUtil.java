...
public class AuthenticationUtil {
	...
	/**
	 * These store a message giving a reason for why the request is being
	 * interrupted.
	 */
	...
	public static Context authenticate(Map objectModel, String email, String password, String realm) throws SQLException {
		...
		int implicitStatus = AuthenticationManager.authenticateImplicit(context, null, null, null, request);
	...
	public static Context authenticateImplicit(Map objectModel)
		...
		if (implicitStatus == AuthenticationMethod.SUCCESS) {
			log.info(LogManager.getHeader(context, "login", "type=implicit"));
			AuthenticationUtil
					.logIn(context, request, context.getCurrentUser());
		}
	...
	/**
	 * Log the given user in as a real authenticated user. This should only be
	 * used after a user has presented credentials and they have been validated.
	 * 
	 * @param context
	 *            DSpace context
	 * @param request
	...
	private static void logIn(Context context, HttpServletRequest request,
		...
		if (!AuthorizeManager.isAdmin(context)
				&& !SystemwideAlerts.canUserStartSession()) {
			// Do not allow this user to login because sessions are being
			// restricted by a systemwide alert.
	 ...
	 * Log the given user in as a real authenticated user. This should only be
	 * used after a user has presented credentials and they have been validated.
	 * This method signature is provided to be easier to call from flow scripts.
	 * 
	 * @param objectModel
	 ...
	 * 
	 */
	public static void logIn(Map objectModel, EPerson eperson)
			throws SQLException {
		final HttpServletRequest request = (HttpServletRequest) objectModel
				.get(HttpEnvironment.HTTP_REQUEST_OBJECT);
		Context context = ContextUtil.obtainContext(objectModel);
	...
	/**
	 * Check to see if there are any session attributes indicating a currently
	 * authenticated user. If there is then log this user in.
	 * 
	 * @param context
	 *            DSpace context
	 * @param request
	...
	public static void resumeLogin(Context context, HttpServletRequest request)
		...
		if (session != null) {
			...
			Integer realid = (Integer) session
					.getAttribute(AUTHENTICATED_USER_ID);

			if (id != null) {
				// Should we check for an ip match from the start of the request
				// to now?
				boolean ipcheck = ConfigurationManager.getBooleanProperty(
						"xmlui.session.ipcheck", true);

				String address = (String) session
						.getAttribute(CURRENT_IP_ADDRESS);
				if (!ipcheck
						|| (address != null && address.equals(request
								.getRemoteAddr()))) {
					...
					// Check to see if systemwide alerts is restricting sessions
					if (!AuthorizeManager.isAdmin(context)
							&& !SystemwideAlerts.canUserMaintainSession()) {
						// Normal users can not maintain their sessions, check
						// to see if this is really an
						...
						if (!administrators.isMember(realEPerson)) {
							// Log this user out because sessions are being
							// restricted by a systemwide alert.
							context.setCurrentUser(null);
							return;
						}
					...
					// Possible hack attempt or maybe your setup is not
					// providing a consistent end-user IP address.
					log.warn(LogManager.getHeader(context, "ip_mismatch", "id="
							+ id + ", request ip=" + request.getRemoteAddr()
							+ ", session ip=" + address));
				}
			} // if id
		} // if session
	...
	/**
	 * Assume the login as another user. Only site administrators may perform
	 * the action.
	 * 
	 * @param context
	 ...
	 *             using an I18nTransformer key as the message
	 */
	public static void loginAs(Context context, HttpServletRequest request,
			EPerson loginAs) throws SQLException, AuthorizeException {
		// Only allow loginAs if the administrator has allowed it.
		if (!ConfigurationManager.getBooleanProperty("webui.user.assumelogin",
				false)) {
			return;
		}
	 ...
	 *            HTTP request
	 */
	public static void logOut(Context context, HttpServletRequest request)
			throws SQLException {
		HttpSession session = request.getSession();

		if (session.getAttribute(EFFECTIVE_USER_ID) != null
				&& session.getAttribute(AUTHENTICATED_USER_ID) != null) {
			Integer effectiveID = (Integer) session
					.getAttribute(EFFECTIVE_USER_ID);
			Integer authenticatedID = (Integer) session
					.getAttribute(AUTHENTICATED_USER_ID);

			if (effectiveID.intValue() != authenticatedID.intValue()) {
				// The user has login in as another user, instead of logging
				// them out,
				// revert back to their previous login name.

				EPerson authenticatedUser = EPerson.find(context,
						authenticatedID);
	...
	/**
	 * Determine if the email can register itself or needs to be created by a
	 * site administrator first.
	 * 
	 * @param objectModel
	 *            The Cocoon object model
	 * @param email
	 ...
	 */
	public static boolean canSelfRegister(Map objectModel, String email)
			throws SQLException {
		final HttpServletRequest request = (HttpServletRequest) objectModel
				.get(HttpEnvironment.HTTP_REQUEST_OBJECT);
		Context context = ContextUtil.obtainContext(objectModel);
		...
		if (SystemwideAlerts.canUserStartSession()) {
			return AuthenticationManager.canSelfRegister(context, request,
					email);
		} else {
	...
	/**
	 * Construct a new, mostly blank, eperson for the given email address. This
	 * should only be called once the email address has been verified.
	 * 
	 * @param objectModel
	 *            The Cocoon object model.
	 * @param email
	 ...
	 */
	public static EPerson createNewEperson(Map objectModel, String email)
	...
	public static boolean isInterupptedRequest(Map objectModel) {
		...
		if (interruptedObject instanceof RequestInfo) {
			// There is currently either an interrupted or yet-to-be resumed
			// request.
			return true;
		}
	 ...
	 *            The Cocoon object Model
	 * @param header
	 *            A message header (i18n tag)
	 * @param message
	 *            A message for why the request was interrupted (i18n tag)
	 * @param characters
	 *            An untranslated message, perhaps an error message?
	 */
	public static void interruptRequest(Map objectModel, String header,
			String message, String characters) {
		final HttpServletRequest request = (HttpServletRequest) objectModel
				.get(HttpEnvironment.HTTP_REQUEST_OBJECT);

		HttpSession session = request.getSession();

		// Store this interrupted request until after the user successfully
		// authenticates.
	...
	/**
	 * Set the interrupted request to a resumable state. The next request that
	 * the server receives (for this session) that has the same servletPath will
	 * be replaced with the previously interrupted request.
	 * 
	 * @param objectModel
	 *            The Cocoon object Model
	 * @return null.
	...
	public static String resumeInterruptedRequest(Map objectModel) {
		...
		session.setAttribute(AuthenticationUtil.REQUEST_INTERRUPTED_HEADER,
				null);
		session.setAttribute(AuthenticationUtil.REQUEST_INTERRUPTED_MESSAGE,
				null);
		session.setAttribute(AuthenticationUtil.REQUEST_INTERRUPTED_CHARACTERS,
				null);
		...
		if (interruptedObject instanceof RequestInfo) {
			...
			if (interruptedRequest.getServletPath() == null
					|| interruptedRequest.getServletPath().length() == 0) {
	 ...
	 * 
	 * @param realHttpRequest
	 *            The current real request
	 * @return Either the current real request or a stored request that was
	...
	public static HttpServletRequest resumeRequest(
		...
		if (object instanceof RequestInfo) {
			...
			if (realServletPath != null
				...
				// Clear the resumed request and send the request back to be
				// resumed.
				session.setAttribute(REQUEST_INTERRUPTED, null);
